...
{#if !readonly}
	...
	<div class="space-y-2">
		<div>
			<label for="{name}-world" class="mb-0.5 block text-xs text-muted-foreground">World</label>
			{#if worlds.length > 0}
				<select
					id="{name}-world"
					bind:value={world}
		...
		<div class="grid grid-cols-5 gap-2">
			{#each COORDS as c}
				<div>
					<label for="{name}-{c.key}" class="mb-0.5 block text-xs text-muted-foreground">{c.label}</label>
					<input
						id="{name}-{c.key}"
						type="number"
						value={vals[c.key]}
...
{:else}
	<span class="text-xs text-muted-foreground font-mono">{fmt(value)}</span>
{/if}
