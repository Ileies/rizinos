...
<div class="{readonly ? 'flex flex-nowrap overflow-hidden gap-1' : 'flex flex-wrap gap-1'}">
	...
	{#if !readonly && !addOpen}
		...
		<button
			...
			class="rounded border border-dashed px-1.5 py-0.5 text-xs text-muted-foreground transition-colors hover:border-foreground hover:text-foreground"
		>+ Add</button>
	{/if}
</div>

{#if !readonly && addOpen}
	<div class="mt-2 space-y-2 rounded-md border bg-muted/30 p-2">
		<div class="flex flex-wrap items-center gap-1.5">
			<div class="flex overflow-hidden rounded border text-xs">
				<button
					type="button"
					onclick={() => (addDeny = false)}
					class="px-2 py-1 transition-colors {!addDeny ? 'bg-green-600 text-white' : 'hover:bg-muted'}"
				>Allow</button>
				<button
					type="button"
					onclick={() => (addDeny = true)}
					class="px-2 py-1 transition-colors {addDeny ? 'bg-destructive text-destructive-foreground' : 'hover:bg-muted'}"
				>Deny</button>
			</div>
			...
			<div class="flex overflow-hidden rounded border text-xs">
				<button
					...
					class="px-2 py-1 transition-colors {addType === 'role' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}"
				>Role</button>
				<button
					type="button"
					onclick={() => (addType = 'user')}
					class="px-2 py-1 transition-colors {addType === 'user' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}"
				>User</button>
			</div>
		</div>
		...
		{#if addType === 'role'}
			<div class="flex items-center gap-1.5">
				<select bind:value={addRole} class="rounded border bg-background px-2 py-1 text-xs">
					{#each ROLES as role}
				...
				>Add</button>
				<button type="button" onclick={resetAdd} class="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-muted">Cancel</button>
			</div>
		{:else}
			<div class="relative">
				<input
				...
				{#if userDropdownOpen && users.length > 0 && filteredUsers.length > 0}
					<ul class="absolute z-50 mt-0.5 w-full overflow-hidden rounded border bg-background shadow-md">
						{#each filteredUsers as u (u.id)}
							<li>
								<button
									type="button"
									class="flex w-full items-center justify-between px-2 py-1.5 text-left text-xs hover:bg-muted {addUserId === u.id ? 'bg-muted font-medium' : ''}"
									onmousedown={(e) => { e.preventDefault(); selectUser(u); }}
								>
									<span>{u.username}</span>
									<span class="font-mono text-muted-foreground">{u.id}</span>
								</button>
							</li>
						{/each}
					</ul>
			...
			<div class="flex items-center gap-1.5">
				...
				{#if addUserId && userSearch}
					<span class="text-xs text-muted-foreground">ID: <span class="font-mono">{addUserId}</span></span>
				{/if}
			</div>
		{/if}
	</div>
