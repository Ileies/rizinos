...
export async function POST(event: RequestEvent) {
	...
	console.log(
		'[mc/saveInventory:8] POST received: uuid=%s, world=%s, inventory type=%s rawLength=%s',
		uuid,
		world,
		typeof inventory,
	...
	try {
		...
		console.log(
			'[mc/saveInventory:15] inventory parsed successfully: %d slots',
	...
	console.log(
		'[mc/saveInventory:25] worldGroup for world="%s":',
		world,
		worldGroup ?? 'not found → returning false'
	);
	...
	console.log(
		'[mc/saveInventory:33] existing record for uuid=%s worldGroup=%s: %s',
		uuid,
		worldGroup,
		exists ? `found (id=${existingId})` : 'not found'
	);
	...
	if (exists) {
		...
		await db
			...
			.set({ inventory: parsedInventory })
			.where(and(eq(mcInventories.ownerUuid, uuid), eq(mcInventories.worldGroup, worldGroup)));
	} else {
		console.log('[mc/saveInventory:42] inserting new inventory record');
		await db
			.insert(mcInventories)
			.values({ ownerUuid: uuid, worldGroup, inventory: parsedInventory });
	}
	console.log(
		'[mc/saveInventory:45] save complete for uuid=%s worldGroup=%s, returning true',
		uuid,
		worldGroup
	);
	return json(true);
