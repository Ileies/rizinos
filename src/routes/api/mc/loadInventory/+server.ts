...
export async function POST(event: RequestEvent) {
	...
	console.log(
		'[mc/loadInventory:21] worldGroup for world="%s":',
		world,
		worldGroup ?? 'not found → returning null'
	);
	...
	console.log(
		'[mc/loadInventory:28] inventory for uuid=%s worldGroup=%s: %s',
		uuid,
		worldGroup,
		inventory ? `${inventory.length} slots` : 'not found, returning empty array (40 slots)'
	);
