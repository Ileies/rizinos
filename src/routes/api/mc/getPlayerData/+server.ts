...
export async function POST(event: RequestEvent) {
	...
	console.log(
		'[mc/getPlayerData:25] db lookup result:',
		data
	...
	if (hasRole(data.user, Role.User))
		['mikosav.user', 'plots.plot.1', 'plots.auto.1', 'plots.permpack.basic'].forEach((i) =>
	...
	console.log(
		'[mc/getPlayerData:55] role injection: was %d, now %d permissions. Added: [%s]',
		permsBefore,
		data.permissions.length,
		addedPerms.join(', ') || 'none'
	);
	...
	console.log(
		'[mc/getPlayerData:67] returning response for username=%s: credit=%s, home=%s, permissions=%d',
		response.username,
		response.credit,
		response.home,
