...
export async function POST(event: RequestEvent) {
	...
	console.log(
		'[mc/canTp:26] mcUsers lookup for uuid=%s:',
		uuid,
		userData ? `found` : 'not found → forbidden'
	);
	...
	if (!toWorld) {
		console.log(
			'[mc/canTp:36] toWorld not found - op=%s → returning %s',
			op,
			op ? 'empty inventory' : 'forbidden'
		);
	...
	console.log(
		'[mc/canTp:43] toWorldGroup lookup for "%s":',
		toWorld.groupName,
		toWorldGroup ?? 'not found → forbidden'
	);
	if (!toWorldGroup) return json({ forbidden: true });

	const accessDenied =
		(!op && !canAccess(userData, toWorld.restrict)) || !canAccess(userData, toWorldGroup.restrict);
	console.log(
		'[mc/canTp:47] access check: op=%s, canAccessWorld=%s, canAccessGroup=%s → denied=%s',
		op,
		canAccess(userData, toWorld.restrict),
		canAccess(userData, toWorldGroup.restrict),
		accessDenied
	);
	if (accessDenied) return json({ forbidden: true });

	const fromWorldGroup = (
		await db.query.mcWorlds.findFirst({
			where: { name: from },
	...
	if (toWorldGroup.name === fromWorldGroup) {
		console.log(
			'[mc/canTp:58] same world group (%s) → keeping inventory, returning null',
			toWorldGroup.name
		);
		return json(null);
	}
	...
	const inventory = (
		await db.query.mcInventories.findFirst({
			columns: { inventory: true },
			where: { ownerUuid: uuid, worldGroup: toWorldGroup.name }
		})
	)?.inventory;
	console.log(
		'[mc/canTp:66] inventory for uuid=%s worldGroup=%s: %s',
		uuid,
		toWorldGroup.name,
		inventory ? `${inventory.length} slots` : 'not found, returning empty (40 slots)'
	);
