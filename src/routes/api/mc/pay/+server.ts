...
export async function POST(event: RequestEvent) {
	const { from, to, amount } = (await event.request.json()) as {
		from: string | null;
		to: string;
		amount: number;
	};
	...
	console.log(
		'[mc/pay:22] toUser lookup for uuid=%s:',
		to,
		toUserData
			? `found (id=${toUserData.id}, username=${toUserData.username})`
	...
	if (!from) {
		console.log(
			'[mc/pay:27] from=null → system purchase: %d diamonds for user %s (id=%s)',
			amount,
			toUserData.username,
			toUserData.id
		);
		try {
			await Transaction.purchase(
				await getSystemUser(),
				toUserData,
				amount * 10,
				'Minecraft diamonds sold'
			);
			console.log('[mc/pay:30] Transaction.purchase succeeded');
			return json(true);
		} catch (error) {
			console.error('[mc/pay:33] Transaction.purchase failed:', {
				amount,
				toId: toUserData.id,
				error
			});
	...
	console.log(
		'[mc/pay:42] fromUser lookup for uuid=%s:',
		from,
		fromUserData
			? `found (id=${fromUserData.id}, username=${fromUserData.username})`
			: 'not found → returning false'
	);
