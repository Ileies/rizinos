...
export async function POST(event: RequestEvent) {
	...
	console.log('[mc/setWarp:18] existing warp lookup for name="%s":', warpName, warp ? 'found' : 'not found');
	...
	if (restrict) {
		...
		console.log('[mc/setWarp:29] restrict resolved: %d input → %d output entries:', restrict.length, data.restrict.length, data.restrict);
	}
	...
	if (warp) {
		if (location || restrict) {
			...
			console.log('[mc/setWarp:39] deleting warp "%s" (no location or restrict provided)', warpName);
			await db.delete(mcWarps).where(eq(mcWarps.name, warpName));
		}
	} else {
		if (!data.location || !data.restrict) {
			console.log('[mc/setWarp:44] cannot create new warp "%s": missing location=%s or restrict=%s',
				warpName, data.location, data.restrict);
			return json(false);
		}
